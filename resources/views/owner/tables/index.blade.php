...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('owner.tables.export-pdf') }}" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Export Semua QR
            </a>
            <a href="{{ route('owner.tables.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">Tambah Meja</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            <div class="bg-gray-50 px-5 py-3 border-t border-gray-100 flex justify-between items-center">
                <a href="{{ route('owner.tables.qr', $table) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800" title="Download QR PDF">
                    Unduh QR
                </a>
                
                <div class="flex items-center gap-3">
                    <a href="{{ route('owner.tables.edit', $table) }}" class="text-gray-500 hover:text-blue-600" title="Edit">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                    </a>
                    
                    <form action="{{ route('owner.tables.regenerate-token', $table) }}" method="POST" onsubmit="return confirm('Yakin ingin reset token QR? QR lama tidak akan bisa dipakai lagi.');">
                        @csrf
                    ...
                    <form action="{{ route('owner.tables.destroy', $table) }}" method="POST" onsubmit="return confirm('Yakin hapus meja ini?');">
                        @csrf
                        @method('DELETE')
